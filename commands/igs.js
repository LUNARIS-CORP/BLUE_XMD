...
async function convertBufferToStickerWebp(inputBuffer, isAnimated, cropSquare) {
  ...
  const json = {
    ...
    'sticker-pack-name': settings.packname || 'BLUE XMD from LUN∆RIS',
    'emojis': ['📸']
  };
...
async function igsCommand(sock, chatId, message, crop = false) {
  try {
    ...
    if (!urlMatch) {
      await sock.sendMessage(chatId, { text: `Envoyez un lien vers une publication/une bobine Instagram.\nUtilisation :\n.igs <url>\n.igsc <url>` }, { quoted: message });
      return;
    }
    ...
    if (!downloadData || !downloadData.data) {
      await sock.sendMessage(chatId, { text: "\u274C \xC9chec de la r\xE9cup\xE9ration des m\xE9dias \xE0 partir du lien Instagram." }, { quoted: message });
      return;
    }
    // Raw items
    ...
    if (items.length === 0) {
      await sock.sendMessage(chatId, { text: "\u274C Aucun m\xE9dia trouv\xE9 sur le lien fourni." }, { quoted: message });
      return;
    }
    ...
    await sock.sendMessage(chatId, { text: "\xC9chec de la cr\xE9ation d'un autocollant \xE0 partir du lien Instagram." }, { quoted: message });
  }
}
