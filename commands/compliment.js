  ...
  try {
    ...
    if (!userToCompliment) {
      await sock.sendMessage(chatId, {
        text: "Merci de mentionner quelqu'un ou de r\xE9pondre \xE0 son message pour le complimenter\xA0!"
      });
      return;
    }
    ...
    await sock.sendMessage(chatId, {
      text: `Salut @${userToCompliment.split('@')[0]}, ${compliment}`,
    ...
    if (error.data === 429) {
      ...
      try {
        await sock.sendMessage(chatId, {
          text: "Veuillez r\xE9essayer dans quelques secondes."
        });
      } catch (retryError) {
      ...
      try {
        await sock.sendMessage(chatId, {
          text: "Une erreur s'est produite lors de l'envoi du compliment."
        });
      } catch (sendError) {
