  ...
  try {
    ...
    if (!userToInsult) {
      await sock.sendMessage(chatId, {
        text: "Merci de mentionner quelqu'un ou de r\xE9pondre \xE0 son message pour l'insulter\xA0!"
      });
      return;
    }
    ...
    await sock.sendMessage(chatId, {
      text: `Salut @${userToInsult.split('@')[0]}, ${insult}`,
    ...
    if (error.data === 429) {
      ...
      try {
        await sock.sendMessage(chatId, {
          text: "Une erreur s'est produite lors de l'envoi de l'insulte."
        });
      } catch (sendError) {
