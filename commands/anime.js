...
async function sendAnimu(sock, chatId, message, type) {
  ...
  if (data.link) {
    ...
    try {
      await sock.sendMessage(
        chatId,
        { image: { url: link }, caption: `animé : ${type}` },
        { quoted: message }
      );
      return;
    } catch {}
  ...
  await sock.sendMessage(
    chatId,
    { text: "\u274C \xC9chec de la r\xE9cup\xE9ration de l'animu." },
    { quoted: message }
  );
}
...
async function animeCommand(sock, chatId, message, args) {
  ...
  try {
    ...
    if (!supported.includes(sub)) {
      await sock.sendMessage(chatId, { text: `❌ Type non pris en charge : ${sub}. Essayez l'un des : ${supported.join(', ')}` }, { quoted: message });
      return;
    }
    ...
    await sock.sendMessage(chatId, { text: "\u274C Une erreur s'est produite lors de la r\xE9cup\xE9ration de l'animu." }, { quoted: message });
  }
}
