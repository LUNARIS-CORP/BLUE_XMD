...
async function handleMentionDetection(sock, chatId, message) {
  try {
    ...
    if (!state.assetPath) {
      await sock.sendMessage(chatId, { text: "Salut" }, { quoted: message });
      return;
    }
    const assetPath = path.join(__dirname, '..', state.assetPath);
    ...
    try {
      ...
      const payload = {};
      if (state.type === 'image') payload.image = fs.readFileSync(assetPath);else
      if (state.type === 'video') {
      ...
      if (state.type === 'text') payload.text = fs.readFileSync(assetPath, 'utf8');else
      payload.text = 'Hi';
...
async function mentionToggleCommand(sock, chatId, message, args, isOwner) {
  if (!isOwner) return sock.sendMessage(chatId, { text: '⚠️ Seul le propriétaire ou le sudo peut utiliser cette commande.' }, { quoted: message });
  const onoff = (args || '').trim().toLowerCase();
  if (!onoff || !['on', 'off'].includes(onoff)) {
    return sock.sendMessage(chatId, { text: 'Utilisation : .mention on|off' }, { quoted: message });
  }
  const state = loadState();
  state.enabled = onoff === 'on';
  saveState(state);
  return sock.sendMessage(chatId, { text: `Réponse aux mentions ${state.enabled ? 'activée' : 'désactivée'}.` }, { quoted: message });
}
...
async function setMentionCommand(sock, chatId, message, isOwner) {
  ...
  if (!qMsg) return sock.sendMessage(chatId, { text: 'Réponds à un message ou à un média (sticker/image/video/audio/document).' }, { quoted: message });

  // Determine type and media key
  let type = 'sticker',buf,dataType;
  if (qMsg.stickerMessage) {dataType = 'stickerMessage';type = 'sticker';} else
  if (qMsg.imageMessage) {dataType = 'imageMessage';type = 'image';} else
  if (qMsg.videoMessage) {dataType = 'videoMessage';type = 'video';} else
  if (qMsg.audioMessage) {dataType = 'audioMessage';type = 'audio';} else
  if (qMsg.documentMessage) {dataType = 'documentMessage';type = 'file';} else
  if (qMsg.conversation || qMsg.extendedTextMessage?.text) {type = 'text';} else
  return sock.sendMessage(chatId, { text: 'Type non pris en charge. Réponds à un texte, sticker, image, vidéo, audio ou document.' }, { quoted: message });

  // Download or capture text
  if (type === 'text') {
    buf = Buffer.from(qMsg.conversation || qMsg.extendedTextMessage?.text || '', 'utf8');
    if (!buf.length) return sock.sendMessage(chatId, { text: "Texte vide." }, { quoted: message });
  } else {
    try {
      ...
      return sock.sendMessage(chatId, { text: '❌ Échec du téléchargement du média.' }, { quoted: message });
    }
  }

  // Size limit 1MB
  if (buf.length > 1024 * 1024) {
    return sock.sendMessage(chatId, { text: '❌ Fichier trop volumineux. Maximum 1 Mo.' }, { quoted: message });
  }
  ...
  let ext = 'bin';
  if (type === 'sticker') ext = 'webp';else
  if (type === 'image') ext = mimetype.includes('png') ? 'png' : 'jpg';else
  if (type === 'video') ext = 'mp4';else
  if (type === 'audio') {
    if (mimetype.includes('ogg') || mimetype.includes('opus')) {ext = 'ogg';mimetype = 'audio/ogg; codecs=opus';} else
    if (mimetype.includes('mpeg') || mimetype.includes('mp3')) {ext = 'mp3';mimetype = 'audio/mpeg';} else
    if (mimetype.includes('aac')) {ext = 'aac';mimetype = 'audio/aac';} else
    if (mimetype.includes('wav')) {ext = 'wav';mimetype = 'audio/wav';} else
    if (mimetype.includes('m4a') || mimetype.includes('mp4')) {ext = 'm4a';mimetype = 'audio/mp4';} else
  ...
  try {fs.writeFileSync(outPath, buf);} catch (e) {
    ...
    return sock.sendMessage(chatId, { text: '❌ Échec de l’enregistrement du fichier.' }, { quoted: message });
  }
  ...
  saveState(state);
  return sock.sendMessage(chatId, { text: "Mention des m\xE9dias de r\xE9ponse mis \xE0 jour." }, { quoted: message });
}
