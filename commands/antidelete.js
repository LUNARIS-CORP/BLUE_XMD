...
async function handleAntideleteCommand(sock, chatId, message, match) {
  ...
  if (!message.key.fromMe && !isOwner) {
    return sock.sendMessage(chatId, { text: "*Seul le propri\xE9taire du bot peut utiliser cette commande.*" }, { quoted: message });
  }
  ...
  if (!match) {
    return sock.sendMessage(chatId, {
      text: `*CONFIGURATION D'ANTIDELETE*\n\nÉtat actuel : ${config.enabled ? '✅ Enabled' : '❌ Disabled'}\n\n*.antidelete activé* - Activer\n*.antidelete désactivé* - Désactiver`
    }, { quoted: message });
  }
  ...
  if (match === 'on') {
    ...
    return sock.sendMessage(chatId, { text: "*Commande invalide. Utilisez .antidelete pour voir l'utilisation.*" }, { quoted: message });
  }

  saveAntideleteConfig(config);
  return sock.sendMessage(chatId, { text: `*Antisuppression ${match === 'on' ? 'enabled' : 'disabled'}*` }, { quoted: message });
}
  ...
  try {
    ...
    if (isViewOnce && mediaType && fs.existsSync(mediaPath)) {
      try {
        ...
        const mediaOptions = {
          caption: `*Anti-ViewOnce ${mediaType}* De : @${
          senderName}`,
          mentions: [sender]
        };
        if (mediaType === 'image') {
...
async function handleMessageRevocation(sock, revocationMessage) {
  try {
    ...
    if (original.mediaType && fs.existsSync(original.mediaPath)) {
      const mediaOptions = {
        caption: `*Supprimé ${original.mediaType}*\nDe : @${senderName}`,
        mentions: [sender]
      };
      ...
      try {
        ...
        await sock.sendMessage(ownerNumber, {
          text: `⚠️ Erreur lors de l'envoi du média : ${err.message}`
        });
      }
