...
async function songCommand(sock, chatId, message) {
  try {
    ...
    if (!text) {
      await sock.sendMessage(chatId, { text: "Utilisation\xA0: .song <nom de la chanson ou lien YouTube>" }, { quoted: message });
      return;
    }
    ...
    if (text.includes('youtube.com') || text.includes('youtu.be')) {
      ...
      if (!search || !search.videos.length) {
        await sock.sendMessage(chatId, { text: "Aucun r\xE9sultat trouv\xE9." }, { quoted: message });
        return;
      }
    ...
    let errorMessage = "\u274C \xC9chec du t\xE9l\xE9chargement de la chanson.";
