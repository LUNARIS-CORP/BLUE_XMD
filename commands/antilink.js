...
async function handleAntilinkCommand(sock, chatId, userMessage, senderId, isSenderAdmin, message) {
  try {
    if (!isSenderAdmin) {
      await sock.sendMessage(chatId, { text: "```Pour les administrateurs de groupe uniquement !```" }, { quoted: message });
      return;
    }
    ...
    switch (action) {
      case 'on':
        ...
        if (existingConfig?.enabled) {
          await sock.sendMessage(chatId, { text: "*_Antilink est d\xE9j\xE0 activ\xE9_*" }, { quoted: message });
          return;
        }
      ...
      case 'off':
        ...
        await sock.sendMessage(chatId, { text: "*_Antilink a \xE9t\xE9 d\xE9sactiv\xE9_*" }, { quoted: message });
        break;

      case 'set':
        if (args.length < 2) {
          await sock.sendMessage(chatId, {
            text: `*_Veuillez spécifier une action : ${prefix}antilink set delete | coup de pied | avertir_*`
          }, { quoted: message });
          return;
        }
        ...
        if (!['delete', 'kick', 'warn'].includes(setAction)) {
          await sock.sendMessage(chatId, {
            text: "*_Action invalide. Choisissez Supprimer, Expulser ou Avertir._*"
          }, { quoted: message });
          return;
        }
      ...
      case 'get':
        ...
        await sock.sendMessage(chatId, {
          text: `*_Configuration Antilink :_*\nStatut : ${status ? 'ON' : 'OFF'}\nAction : ${actionConfig ? actionConfig.action : 'Not set'}`
        }, { quoted: message });
        break;

      default:
        await sock.sendMessage(chatId, { text: `*_Utilisez ${prefix}antilink pour l'utilisation._*` });
    }
  } catch (error) {
    console.error('Error in antilink command:', error);
    await sock.sendMessage(chatId, { text: "*_Erreur lors du traitement de la commande antilink_*" });
  }
}
...
async function handleLinkDetection(sock, chatId, message, userMessage, senderId) {
  ...
  if (shouldDelete) {
    ...
    await sock.sendMessage(chatId, { text: `Avertissement! @${senderId.split('@')[0]}, la publication de liens n'est pas autorisée.`, mentions: mentionedJidList });
  } else {
