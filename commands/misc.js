...
async function handleHeart(sock, chatId, message) {
  try {
    ...
    await sock.sendMessage(chatId, { text: "\u274C \xC9chec de la cr\xE9ation de l'image du c\u0153ur. R\xE9essayez plus tard." }, { quoted: message });
  }
}
  ...
  try {
    switch (sub) {
      ...
      case 'its-so-stupid':{
          ...
          if (!dog) {
            await sock.sendMessage(chatId, { text: "Utilisation\xA0: .misc c'est tellement stupide <texte>" }, { quoted: message });
            return;
          }
          const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
      ...
      case 'namecard':{
          ...
          if (!username || !birthday) {
            await sock.sendMessage(chatId, { text: "Utilisation\xA0:\xA0nom d'utilisateur .misc namecard|anniversaire|description (facultatif)" }, { quoted: message });
            return;
          }
          const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
      ...
      case 'oogway2':{
          ...
          if (!quote) {
            await sock.sendMessage(chatId, { text: `Utilisation : .misc ${sub} <quote>` }, { quoted: message });
            return;
          }
          const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
      ...
      case 'tweet':{
          ...
          if (!displayname || !username || !comment) {
            await sock.sendMessage(chatId, { text: "Utilisation\xA0: .misc tweet displayname|username|comment|theme (facultatif clair/fonc\xE9)" }, { quoted: message });
            return;
          }
          const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
      ...
      default:
        await sock.sendMessage(chatId, { text: "Utilisation\xA0: .misc <heart|horny|circle|lgbt|lesbian|nonbinary|pansexual|transgender|lied|lolice|simpcard|tonikawa|its-so-stupid <text>|namecard u|b|d?|nobitches <text>|oogway <q>|oogway2 <q>|tweet dn|un|c|theme?|youtube-comment un|c>" }, { quoted: message });
        break;
    }
  } catch (error) {
    console.error('Error in misc command:', error);
    await sock.sendMessage(chatId, { text: "\u274C \xC9chec de la g\xE9n\xE9ration de l'image. V\xE9rifiez vos param\xE8tres et r\xE9essayez." }, { quoted: message });
  }
}
