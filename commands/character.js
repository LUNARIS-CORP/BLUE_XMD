...
async function characterCommand(sock, chatId, message) {
  ...
  if (!userToAnalyze) {
    await sock.sendMessage(chatId, {
      text: 'Mentionne quelqu’un ou réponds à son message pour analyser son caractère !',
      ...channelInfo
    });
    return;
  }
  ...
  try {
    ...
    const analysis = `🔮 *Analyse du caractère* 🔮\n\n` +
    `👤 *Utilisateur :* ${userToAnalyze.split('@')[0]}\n\n` +
    `✨ *Traits principaux :*\n${traitPercentages.join('\n')}\n\n` +
    `🎯 *Note globale :* ${Math.floor(Math.random() * 21) + 80}%\n\n` +
    `Note : c’est une analyse ludique et ne doit pas être prise au sérieux !`;
    ...
    await sock.sendMessage(chatId, {
      text: '❌ Impossible d’analyser le caractère. Réessaie plus tard.',
      ...channelInfo
    });
  }
}
