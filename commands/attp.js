...
async function attpCommand(sock, chatId, message) {
  ...
  if (!text) {
    await sock.sendMessage(chatId, { text: "Veuillez fournir du texte apr\xE8s la commande .attp." }, { quoted: message });
    return;
  }

  try {
    const mp4Buffer = await renderBlinkingVideoWithFfmpeg(text);
    const webpPath = await writeExifVid(mp4Buffer, { packname: 'BLUE XMD' });
    ...
    await sock.sendMessage(chatId, { text: "\xC9chec de la g\xE9n\xE9ration locale de l'autocollant." }, { quoted: message });
  }
}
