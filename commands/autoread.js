  ...
  try {
    ...
    if (!message.key.fromMe && !isOwner) {
      await sock.sendMessage(chatId, {
        text: "\u274C Cette commande n'est disponible que pour le propri\xE9taire !",
        contextInfo: {
          forwardingScore: 1,
          isForwarded: true,
    ...
    if (args.length > 0) {
      ...
      if (action === 'on' || action === 'enable') {
        ...
        await sock.sendMessage(chatId, {
          text: "\u274C Option invalide ! Utilisation\xA0: .autoread activ\xE9/d\xE9sactiv\xE9",
          contextInfo: {
            forwardingScore: 1,
            isForwarded: true,
            forwardedNewsletterMessageInfo: {
    ...
    await sock.sendMessage(chatId, {
      text: `✅ La lecture automatique a été ${config.enabled ? 'enabled' : 'disabled'} !`,
      contextInfo: {
        forwardingScore: 1,
        isForwarded: true,
        forwardedNewsletterMessageInfo: {
    ...
    await sock.sendMessage(chatId, {
      text: "\u274C Erreur lors du traitement de la commande\xA0!",
      contextInfo: {
        forwardingScore: 1,
        isForwarded: true,
        forwardedNewsletterMessageInfo: {
...
function isBotMentionedInMessage(message, botNumber) {
  ...
  if (textContent) {
    ...
    const botNames = [global.botname?.toLowerCase(), 'bot', 'BLUE', 'LUN∆RIS'];
