...
async function kickCommand(sock, chatId, senderId, mentionedJids, message) {
  ...
  if (!isOwner) {
    ...
    if (!isSenderAdmin) {
      await sock.sendMessage(chatId, { text: "Seuls les administrateurs de groupe peuvent utiliser la commande kick." }, { quoted: message });
      return;
    }
  }
  ...
  if (usersToKick.length === 0) {
    await sock.sendMessage(chatId, {
      text: "Veuillez mentionner l'utilisateur ou r\xE9pondre \xE0 son message pour kicker\xA0!"
    }, { quoted: message });
    return;
  }
  ...
  if (isTryingToKickBot) {
    await sock.sendMessage(chatId, {
      text: "Je ne peux pas me donner un coup de pied\uD83E\uDD16"
    }, { quoted: message });
    return;
  }
  ...
  try {
    ...
    await sock.sendMessage(chatId, {
      text: "\xC9chec de l'expulsion des utilisateurs\xA0!"
    });
  }
}
